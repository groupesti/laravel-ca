...
final class CaManager
{
    ...
    public function createRootCA(
        ...
        $this->logOperation('ca_creation', 'info', "Root CA created: {$dn->commonName}", [
            'ca_id' => $ca->id,
    ...
    public function createIntermediateCA(
        ...
        if (!$parent->status->isValid()) {
            $e = new CaException("Parent CA [{$parent->id}] is not in a valid state.");
            $this->logException($e, 'ca_creation', ['parent_id' => $parent->id]);
            throw $e;
        }

        if ($parent->path_length !== null && $parent->path_length <= 0) {
            $e = new CaException("Parent CA [{$parent->id}] does not allow further subordinate CAs.");
            $this->logException($e, 'ca_creation', ['parent_id' => $parent->id]);
            throw $e;
        }
        ...
        if ($requestedNotAfter->greaterThan($parentNotAfter)) {
            $e = new CaException('Intermediate CA validity cannot exceed parent CA validity.');
            $this->logException($e, 'ca_creation', ['parent_id' => $parent->id]);
            throw $e;
        }
        ...
        $this->logOperation('ca_creation', 'info', "Intermediate CA created: {$dn->commonName}", [
            'ca_id' => $ca->id,
            'parent_id' => $parent->id,
            'algorithm' => $algorithm->slug,
            'validity_days' => $validityDays,
    ...
    public function suspend(CertificateAuthority $ca): void
    {
        if ($ca->status === CertificateStatus::REVOKED) {
            $e = new CaException("Cannot suspend a revoked CA [{$ca->id}].");
            $this->logException($e, 'ca_suspend', ['ca_id' => $ca->id]);
            throw $e;
        }
        ...
        $this->logOperation('ca_suspend', 'warning', "CA suspended: {$ca->id}", ['ca_id' => $ca->id]);
    ...
    {
        if ($ca->status === CertificateStatus::REVOKED) {
            $e = new CaException("Cannot activate a revoked CA [{$ca->id}].");
            $this->logException($e, 'ca_activate', ['ca_id' => $ca->id]);
            throw $e;
        }

        if ($ca->status === CertificateStatus::EXPIRED) {
            $e = new CaException("Cannot activate an expired CA [{$ca->id}].");
            $this->logException($e, 'ca_activate', ['ca_id' => $ca->id]);
            throw $e;
        }
        ...
        if ($ca->parent_id !== null) {
            ...
            if ($parent !== null && !$parent->status->isValid()) {
                ...
                $this->logException($e, 'ca_activate', ['ca_id' => $ca->id, 'parent_id' => $parent->id]);
                throw $e;
            }
        }
        ...
        $this->logOperation('ca_activate', 'info', "CA activated: {$ca->id}", ['ca_id' => $ca->id]);
    }
